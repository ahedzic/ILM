...
class HGAT_Meta(nn.Module):
    ...
    def forward(self, g, gpu_id):
        ...
        if torch.isnan(g.ndata['z']).any():
            print("Feat is NaN", flush=True)
            return 0
        h = self.layer1(g)
        if torch.isnan(h).any():
            print("H1 is NaN", g.ndata['z'].max(), g.ndata['z'].min(), flush=True)
            return 0
        h = F.leaky_relu(h)
        g.ndata['z'] = h
        h = self.layer2(g)
        if torch.isnan(h).any():
            print("H2 is NaN", flush=True)
            return 0
        # Whole graph embedding
        self.graph_embedding = torch.zeros(self.graph_dim).to('cuda:' + gpu_id)
        self.node_embeddings = self.graph_fc2(F.leaky_relu(self.graph_fc(h)))

        if torch.isnan(self.node_embeddings).any():
            print("Nod embeddings is NaN", flush=True)
            return 0
        ...
        if norm > 0.0:
            ...
            self.graph_embedding_normalized = self.graph_embedding

        if torch.isnan(norm).any():
            print("HGAT NORM embeddings is NaN", flush=True)
            return 0
        if torch.isnan(self.graph_embedding_normalized).any():
            print("HGAT GRAPH embeddings is NaN", flush=True)
            return 0

        return self.graph_embedding_normalized, self.node_embeddings