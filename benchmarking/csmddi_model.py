...
class BaseModelTrain():
    ...
    def init(self, train_drugs_indices, test_drugs_indices):
        ...
        self.adj_train = np.zeros(
            (self.train_drugs_num, self.train_drugs_num), dtype=int)
        for i in range(self.adj_train.shape[0]):
            for j in range(i+1, self.adj_train.shape[1]):
                self.adj_train[i, j] = self.data['adj'][train_drugs_indices[i], train_drugs_indices[j]]
    ...
    def binary_test_S2(self,):
        ...
        return label_true, score_predict_matrices
    ...
    def feature_map_to_embedding(self,):
        module = importlib.import_module('feature_learning')
        map_model = getattr(module, self.config['map_model'])()

        train_feature = self.data['x'][self.train_drugs_indices]
        map_model.fit(train_feature, self.train_embedding)

        test_features = self.data['x'][self.test_drugs_indices]
...
class ColdStartRelationLearning(BaseModelTrain):
    ...
    def train(self,):
        module = importlib.import_module('relation_learning')
...
class ColdStartDeepDDI(BaseModelTrain):
    def __init__(self, config, data):
        ...
        self.data.feature = Jaccard(self.data.feature)
...
class ColdStartDDIMDL(BaseModelTrain):
    def __init__(self, config, data):
        ...
        for feature in self.data.features:
            ...
            feature = PCA(n_components=500).fit_transform(feature)
