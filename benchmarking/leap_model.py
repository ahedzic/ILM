...
class Model(nn.Module):
    def __init__(self, input_dim, num_targets=100, output_dim=128, dropout=0.1, device=None):
        super().__init__()
    ...
    def encode(self, x, edge_index, edge_weight=None, target = False):
        if edge_weight is None:
            x = (self.conv2.encode(x, edge_index=edge_index, edge_weight = torch.ones(edge_index.shape[1]).to(self.device)))
        else:  # leap without augmented edges
...
#             x = F.dropout(x, p=self.dropout, training=self.training)
    ...
    def forward(self, x, message_edge_index=None, target_edge_index=None, target_edge_weights = None, mlp_inputs=None):
        """
        x.shape = [N, D]
        ...
        if target_edge_index is None:
            return self.encode(x, message_edge_index, torch.ones(message_edge_index.shape[1]).to(self.device)), target_edge_weights
        ...
        #target_edge_weights = torch.ones(target_edge_index.shape[1]).to(self.device)
        
        if message_edge_index != None:
            edge_index = torch.cat([message_edge_index, target_edge_index], dim = 1)
            edge_weight = torch.cat((torch.ones(message_edge_index.shape[1]).to(self.device), target_edge_weights))
        else:
            edge_index = target_edge_index
            edge_weight = target_edge_weights
                  
        out = F.rrelu(self.encode(x, edge_index, edge_weight) + self.encode(x, target_edge_index.long(), target_edge_weights)) 
        #out = (self.encode(x, message_edge_index)) # leap without augmented edges
        return out, target_edge_weights

    
    
    def loss(self, edge_index, num_nodes, z: torch.Tensor, variational=True):
        loss = self.conv2.recon_loss(z, edge_index) 
        if variational:
            loss = loss + (1 / num_nodes) * self.conv2.kl_loss()
