    ...
    for graphs_key in graphs.keys():
        for graph in graphs[graphs_key]:
            ...
            if (len(train_pos) > 0) and (len(train_neg) > 0):
                ...
                train_size = int(0.85 * len(positive_edges))
                test_size = int(0.15 * len(positive_edges))
                pos_train_edges = positive_edges[:train_size]
                neg_train_edges = negative_edges[:train_size]
                pos_test_edges = positive_edges[train_size:train_size+test_size]
                neg_test_edges = negative_edges[train_size:train_size+test_size]
                print(pos_test_edges, neg_test_edges)
