...
def read_data(data_name, dir_path, cold_perc, blind, model, n, groups, device):
    ...
    for graphs_key in graphs.keys():
        #data_indices[graphs_key] = n_total
        #data_indices_list.append((graphs_key, n_total))
        ...
        for graph in graphs[graphs_key]:
            ...
            if (len(train_pos) > 0) and (len(train_neg) > 0):
                #full_edges = torch.cat((torch.tensor(given_edges), torch.tensor(positive_edges)), dim=0)
                ...
                #full_adj_edge = torch.transpose(torch.tensor(full_edges.clone().detach(), dtype=torch.long), 1, 0)
                ...
                #n_total += graph['node_count']

                if model == 'ACCSLP':
                    x = graph['gnn_feature']
                    # Calculate attribute similarity information using cosine similarity
                    Z = torch.zeros((graph['node_count'], graph['node_count']))
                    e = 1.0e-10
                    ...
                    X = torch.zeros((graph['node_count'], graph['node_count']))

                    for i in range(len(node_membership)):
                        for j in range(len(node_membership)):
                            if i != j and node_membership[i] == node_membership[j]:
                                X[i][j] = 1.0
                                X[j][i] = 1.0
    ...
    '''train_S = torch.zeros(n_total, n_total).to(device)
    full_S = torch.zeros(n_total, n_total).to(device)
    full_X = torch.zeros(n_total, n_total).to(device)
    full_Z = torch.zeros(n_total, n_total).to(device)

    for graph_tuple in data_indices_list:
        start_index = graph_tuple[1]

        for graph in data[graph_tuple[0]]:
            node_count = graph['node_count']
            if graph_tuple[0] == 'train':
                full_S[start_index:start_index+node_count,start_index:start_index+node_count] = graph['full_adj'].to_dense()
                train_S[start_index:start_index+node_count,start_index:start_index+node_count] = graph['full_adj'].to_dense()
            else:
                full_S[start_index:start_index+node_count,start_index:start_index+node_count] = graph['full_adj'].to_dense()
                train_S[start_index:start_index+node_count,start_index:start_index+node_count] = graph['adj'].to_dense()
            full_X[start_index:start_index+node_count,start_index:start_index+node_count] = graph['community_membership']
            full_Z[start_index:start_index+node_count,start_index:start_index+node_count] = graph['attribute_matrix']
            start_index += node_count

    data['train_S'] = train_S
    data['full_S'] = full_S
    data['full_X'] = full_X
    data['full_Z'] = full_Z
    ...
    for run in range(args.runs):
        ...
        for epoch in range(0, args.epochs):
            ...
            for graph in data['test']:
                model = eval(args.model)(graph['node_count'], args.rank, args.alpha, args.beta, args.groups, device).to(device)
