...
def train(model, score_func, graph, optimizer, device, iterations):
    ...
    for _ in range(iterations):
        ...
        h = model(x, adj)
        edge = expected_pos
        pos_out, pos_add_score = score_func(x, h, edge[0], edge[1], _, node_density, clustering_coeffs, graph_assortativity)
        pos_loss = -torch.log(pos_out + 1e-15).mean()
        edge = expected_neg
        neg_out, neg_add_score = score_func(x, h, edge[0], edge[1], _, node_density, clustering_coeffs, graph_assortativity)
...
def test_edge(score_func, graph, pos_edges, neg_edges, model, device, iterations):
    ...
    for _ in range(iterations):
        ...
        h = model(x, adj)
        edges = pos_edges
        pos_scores, pos_add_score = score_func(x, h, edges[0], edges[1], _, node_density, clustering_coeffs, graph_assortativity)
        pos_scores = pos_scores.cpu()

        edges = neg_edges
        neg_scores, neg_add_score = score_func(x, h, edges[0], edges[1], _, node_density, clustering_coeffs, graph_assortativity)
