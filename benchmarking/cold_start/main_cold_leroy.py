...
def read_data(data_name, dir_path, cold_perc, blind, model, device):
    if cold_perc > 0.0:
        if cold_perc == 0.25:
            cold_part = '25'
        if cold_perc == 0.50:
    ...
    for graphs_key in graphs.keys():
        for graph in graphs[graphs_key]:
            ...
            if (len(train_pos) > 0) and (len(train_neg) > 0):
                ...
                if model == 'Leroy':
                    x = graph_data['x']
                    groups_size = torch.zeros((len(x[0]), 1))
                    g_max = x.max().item()
                    g_min = x.min().item()
                    ...
                    # Create node groups from node features
                    node_groups = torch.where(x >= group_thresh, 1.0, 0.0)
                    groups_size = torch.sum(node_groups, dim=0)
                    graph_data['node_groups'] = node_groups
                    graph_data['groups_size'] = groups_size
...
def main():
    ...
    data = read_data(args.data_name, args.input_dir, args.cold_perc, args.blind, args.model, args.device)
