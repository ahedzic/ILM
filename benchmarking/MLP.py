...
class MLPSamples:
    ...
    def __init__(self, edge_index, num_inputs=10, num_targets=100, device=None):
        self.num_inputs = num_inputs
        self.num_targets = num_targets
        self.g = nx.Graph(edge_index.cpu().numpy().T.tolist())
        self.device = device
        
    @classmethod
    def create_mapping(cls, ids): 
        return dict(zip(ids.cpu().numpy(), range(len(ids)))), ids
    ...
    def _compute_input_to_target_distance(self):
        self.shortest_path_lengths = torch.zeros(self.num_inputs, self.num_targets).to(self.device)       
    ...
    def _create_target_edges(self):
        ...
        for node in self.inputs:
            edges = zip([node] * self.targets.shape[0], self.targets)
