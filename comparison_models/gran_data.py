...
class GRANData(object):
  ...
  def __init__(self, config, graphs, true_graphs, tag='train'):
    self.config = config
    ...
    self.node_order = config["node_order"]
    self.num_canonical_order = config["num_canonical_order"]
    self.tag = tag
    self.num_fwd_pass = config["num_fwd_pass"]
    self.is_sample_subgraph = config["is_sample_subgraph"]
    self.num_subgraph_batch = config["num_subgraph_batch"]
    self.is_overwrite_precompute = config["is_overwrite_precompute"]
    ...
    self.save_path = os.path.join(
        self.data_path, '{}_{}_{}_{}_{}_{}_{}_precompute'.format(
            config["model"], config["name"], tag, self.block_size,
            self.stride, self.num_canonical_order, self.node_order))

    if not os.path.isdir(self.save_path) or self.is_overwrite_precompute:
      self.file_names = []
      self.true_file_names = []
      self.node_file_names = []
      if not os.path.isdir(self.save_path):
        os.makedirs(self.save_path)
      if not os.path.isdir(self.save_path+"_feat"):
        os.makedirs(self.save_path+"_feat")
      if not os.path.isdir(self.save_path+"_true"):
        os.makedirs(self.save_path+"_true")

      self.config["save_path"] = self.save_path
      
      for index in tqdm(range(self.num_graphs)):
        G = self.graphs[index]
        G_true = self.true_graphs[index]
        #label = self.labels[index]
        data, node_feat = self._get_graph_data(G)
        adj_true_list, node_feat = self._get_graph_data(G_true)
        tmp_path = os.path.join(self.save_path, '{}_{}.p'.format(tag, index))
        pickle.dump(data, open(tmp_path, 'wb'))
        self.file_names += [tmp_path]
        tmp_path = os.path.join(self.save_path+"_true", '{}_{}.p'.format(tag, index))
        pickle.dump(adj_true_list, open(tmp_path, 'wb'))
        self.true_file_names += [tmp_path]
        tmp_path = os.path.join(self.save_path+"_feat", '{}_{}.p'.format(tag, index))
        pickle.dump(node_feat, open(tmp_path, 'wb'))
        self.node_file_names += [tmp_path]
    else:
      self.file_names = glob.glob(os.path.join(self.save_path, '*.p'))
      self.node_file_names = glob.glob(os.path.join(self.save_path+"_feat", '*.p'))
      self.true_file_names = glob.glob(os.path.join(self.save_path+"_true", '*.p'))
  ...
  def _get_graph_data(self, G):
    ...
    feat = np.zeros((self.max_num_nodes, self.config["node_feature_size"]))
    nx_feat = nx.get_node_attributes(G, 'feat')
    ...
    #TODO VERIFY CONVERSION TO UNDIRECTED IS OK HERE
    ### BFS & DFS from largest-degree node
    CGs = [G.subgraph(c) for c in nx.connected_components(G.to_undirected())]
    ...
    ### k-core
    num_core = nx.core_number(nx.Graph(G))
  ...
  def __getitem__(self, index):
    ...
    adj_true_list = pickle.load(open(self.true_file_names[index], 'rb'))
    feat_list = pickle.load(open(self.node_file_names[index], 'rb'))
    ...
    for ff in range(self.num_fwd_pass):
      ...
      ### pack tensors
      data = {}
      data['adj'] = torch.from_numpy(np.tril(np.stack(adj_list, axis=0), k=-1))
      data['adj_true'] = torch.from_numpy(np.tril(np.stack(adj_true_list, axis=0), k=-1))
      data['node_feat'] = torch.from_numpy(np.stack(feat_list, axis=0))
      data['edges'] = torch.cat(edges, dim=1).t().long()
      data['node_idx_gnn'] = torch.from_numpy(np.concatenate(node_idx_gnn))
      data['node_idx_feat'] = torch.from_numpy(np.concatenate(node_idx_feat))
      data['label'] = torch.from_numpy(np.concatenate(label))
      data['att_idx'] = torch.from_numpy(np.concatenate(att_idx))
      data['subgraph_idx'] = torch.from_numpy(np.concatenate(subgraph_idx))
  ...
  def collate_fn(self, batch):
    ...
    for ff in range(self.num_fwd_pass):
      ...
      data['adj_true'] = torch.from_numpy(
          np.stack(
              [
                  np.pad(
                      bb['adj_true'], ((0, 0), (0, pad_size[ii]), (0, pad_size[ii])),
                      'constant',
                      constant_values=0.0) for ii, bb in enumerate(batch_pass)
              ],
              axis=0)).float()  # B X C X N X N
      ...
      data['node_feat'] = torch.from_numpy(
          np.stack(
              [
                  bb['node_feat'] for bb in batch_pass
              ],
              axis=0)).float()  # B X C X N X H
