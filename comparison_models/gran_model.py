...
class GNN(nn.Module):
  ...
  def _prop(self, state, edge, edge_feat, layer_idx=0):
    ...
    if self.edge_feat_dim > 0:
      #print(state_diff.shape, edge_feat.shape, flush=True)
...
class GRANMixtureBernoulli(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.is_sym = config['is_sym']
    self.block_size = config['block_size']
    self.sample_stride = config['sample_stride']
    self.num_GNN_prop = config['num_GNN_prop']
    self.num_GNN_layers = config['num_GNN_layers']
    self.edge_weight = config['edge_weight']
    self.dimension_reduce = config['dimension_reduce']
    self.has_attention = config['has_attention']
    ...
    self.att_edge_dim = config["num_edge_types"]
  ...
  '''def _inference(self,
    ...
    #print("BEFORE node_feat", node_feat.shape)
    ### GNN inference
    # pad zero as node feature for newly generated nodes (1st row)
    node_feat = F.pad(
        node_feat[0], (0, 0, 1, 0), 'constant', value=0.0)  # (BCN_max + 1) X N_max
    #print("AFTER node_feat", node_feat.shape)
    ...
    if self.has_rand_feat:
      ...
      #print("att_edge_feat", att_edge_feat.shape, "att_idx", att_idx.shape, "edges", edges.shape)
      #print(att_idx[[edges[:, 0]]], (att_idx[[edges[:, 1]]] + self.att_edge_dim), self.att_edge_dim)
      #print(att_idx[[edges[:, 0]]].shape, (att_idx[[edges[:, 1]]] + self.att_edge_dim).shape, self.att_edge_dim)
    ...
    #print("node_feat", node_feat.shape, "edges", edges.shape, "edge_feat", att_edge_feat.shape)
  ...
  def _sampling(self, A_pad, node_feat=None):
    """ generate adj in row-wise auto-regressive fashion """
    with torch.no_grad():
        B, C, N_max, _ = A_pad.shape
        K = self.block_size
        ...
        ### cache node state for speed up
        #node_state = torch.zeros(B, N_pad, dim_input).to(self.device)
        #print("NPAD", N_pad, "S", S, "A", A.shape)
        for ii in range(0, N_pad, S):
          # for ii in range(0, 3530, S):
          jj = ii + K
          if jj > N_pad:
          ...
          if self.has_rand_feat:
            ...
            att_edge_feat = torch.zeros(edges.shape[0],
                                        2 * self.att_edge_dim).to(self.device)
            att_edge_feat = att_edge_feat.scatter(1, att_idx[[edges[:, 0]]], 1)
            #print("Attk index", att_idx.shape, "ATT EDGE DIM", self.att_edge_dim, "EDGES", edges.shape)
            att_edge_feat = att_edge_feat.scatter(
                1, att_idx[[edges[:, 1]]] + self.att_edge_dim, 1)
          node_state_out = self.decoder(
              node_state_in[0].view(-1, H), edges, edge_feat=att_edge_feat)
          node_state_out = node_state_out.view(B, jj, -1)
  ...
  def forward(self, input_dict):
    ...
    node_feat = input_dict['node_feat'] if 'node_feat' in input_dict else None
    ...
    if not is_sampling:
      ...
      return adj_loss'''
      A = self._inference(A_pad, node_feat)
      ...
      A = self._sampling(A_pad, node_feat)
